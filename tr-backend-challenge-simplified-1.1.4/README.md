...
It will receive updates from a partner processor, transform these updates and provide the aggregated data through an endpoint.
...
### Input data
The input data is received through a websocket stream from a partner processor.
...
End-users of the processor are interested in a specific instruments price history, and they want it in a format that is easy to read.
...
**Your task is to implement a processor that will consume the stream data and make it reachable through the API in the format mentioned above**
...
- `Timestamp on Receipt`: Since event timestamps are missing, I use the time when the event reaches the service as the timestamp
- `Synchronized Events`: I assume both quote and instrument events are in sync and ordered.
- `Single Quote Handling`: If only one quote is received in a minute, its prices (open, close, high, low) will all be the same.
- `Cleanup of Old Events`: Quotes older than 30 minutes are not required, so the cleanup processor removes them, with a 2-minute buffer to handle filling if needed.
- `Missing Candlestick`: If no previous candlestick is available, I assume it's missing.

# Future Enhancements

- `Configurable Properties`: Currently, the time window (30 minutes) is hardcoded due to time constraints. It should be made configurable for flexibility.
- `Enhanced Monitoring & Logging`: Add metrics like event processing time, success/failure rates, and system performance indicators, along with detailed logging for better monitoring and debugging.
- `Persistent Storage`: At the moment, I'm using an in-memory solution. If persistence is required for instruments/quotes, we can explore integrating a proper storage system.
- `Improved Decoupling`: While I've decoupled the producer and consumer within the application, achieving a more reliable, fault-tolerant, and scalable solution would require component-level decoupling.
- `Candlestick Persistence`: Persisting computed candlesticks for future use could improve efficiency and avoid unnecessary re-computation.
- Decouple QuotesCleaner from QuotesRepository

# Setup 

You can either choose to use the framework we provide you, or to use your own framework.
For the latter, see the specification information about the partner processor at the end of this section.
...
To run a partner processor you can either use docker-compose. Docker v3 or above will require slight changes to the docker-compose files.
``` 
docker-compose up -d
```
or Java
```
java -jar partner-processor-1.0.1-all.jar --port=8032
```
...
```
Note: If you don't implement your processor, you will see an exception here
...
#### PartnerService
This coding challenge includes a runnable JAR (the partner processor) that provides the websockets mentioned below.
